"""Gemma 3 KV-cache prefill-then-decode test on the first global layer."""
...
def test_prefill_then_decode_gemma3_layer5_global(hf_model):
    """First global layer (idx 5) — exercises θ=1_000_000."""
    cfg = g3c.Gemma3Config.from_hf_dict(hf_model.config.to_dict())
    api_blk = g3l.build_gemma3_decoder_layer(cfg, layer_idx=5, max_seq=64)
    g3l.load_hf_gemma3_layer(api_blk, hf_model.state_dict(), layer_idx=5)
    api_blk.eval()
