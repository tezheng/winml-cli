...
as HF for SmolLM3-3B (RoPE layer).

Layer 0 is a RoPE layer (no_rope_layers[0] == 1). The NoPE-aware path is
already covered by `test_numerical_hf.py::test_layer_forward_matches_hf[3]`.
"""
import os
import pytest
import torch
...
def test_prefill_then_decode_layer0(hf_model):
    ...
    s_layer.load_hf_smollm3_layer(api_blk, hf_model.state_dict(), layer_idx=0)
    api_blk.eval()
    ...
    print(f"SmolLM3-3B prefill max_abs_diff={pre_diff:.3e}, decode max_abs_diff={dec_diff:.3e}")
