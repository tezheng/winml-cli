...
def _config_tiny():
    return config.Smollm3Config(
        ...
        head_dim=16, intermediate_size=128, num_hidden_layers=8,
        ...
        no_rope_layers=tuple([1, 1, 1, 0, 1, 1, 1, 0]),
    )
...
def test_load_hf_weights_rope_layer():
    cfg = _config_tiny()
    blk = layer.build_smollm3_decoder_layer(cfg, layer_idx=0, max_seq=32)
    sd = _synthetic_hf_state_dict(cfg, layer_idx=0)
    layer.load_hf_smollm3_layer(blk, sd, layer_idx=0)
...
def test_load_hf_weights_nope_layer():
    """NoPE layers have the same projection tensor names — only RoPE module
    is absent. The loader should not complain about missing rope-cache buffers.
    """
    cfg = _config_tiny()
    blk = layer.build_smollm3_decoder_layer(cfg, layer_idx=3, max_seq=32)
    assert blk.attention.rope is None  # sanity check
    sd = _synthetic_hf_state_dict(cfg, layer_idx=3)
    layer.load_hf_smollm3_layer(blk, sd, layer_idx=3)
    assert torch.allclose(
        blk.attention.q_proj.weight, sd["model.layers.3.self_attn.q_proj.weight"]
    )
