"""Verify our ContiguousKVCache prefill+decode matches HF for Phi-4-mini-instruct."""
import os
import pytest
import torch
...
def hf_model():
    ...
    if not _phi4_weights_complete(cache_dir):
        pytest.skip("Phi-4-mini weights not fully downloaded")
    try:
        ...
        pytest.skip(f"Phi-4-mini load failed: {type(e).__name__}: {str(e)[:200]}")
    model.eval()
    return model
...
def test_prefill_then_decode_layer0(hf_model):
    ...
    p_layer.load_hf_phi4_mini_layer(api_blk, hf_model.state_dict(), layer_idx=0)
    api_blk.eval()
    ...
    print(f"Phi-4-mini prefill max_abs_diff={pre_diff:.3e}, decode max_abs_diff={dec_diff:.3e}")
