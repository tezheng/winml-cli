"""Verify our ContiguousKVCache prefill+decode matches HF for Phi-3-mini-4k."""
import os
import pytest
import torch
...
def hf_model():
    ...
    if not _phi3_weights_complete(cache_dir):
        pytest.skip("Phi-3-mini-4k weights not fully downloaded")
    try:
        ...
        pytest.skip(f"Phi-3-mini-4k load failed: {type(e).__name__}: {str(e)[:200]}")
    model.eval()
    return model
...
def test_prefill_then_decode_layer0(hf_model):
    ...
    p_layer.load_hf_phi3_mini_layer(api_blk, hf_model.state_dict(), layer_idx=0)
    api_blk.eval()
    ...
    with torch.no_grad():
        ...
        api_pre_out = api_blk(
            api_prefill_h, position_ids=torch.arange(S_pre),
            cache=api_cache, start_pos=0,
        )
        api_dec_out = api_blk(
            api_decode_h, position_ids=torch.tensor([S_pre]),
            cache=api_cache, start_pos=S_pre,
        )

    pre_diff = (hf_pre_out - api_pre_out).abs().max().item()
    dec_diff = (hf_dec_out - api_dec_out).abs().max().item()
    print(f"Phi-3-mini-4k prefill max_abs_diff={pre_diff:.3e}, decode max_abs_diff={dec_diff:.3e}")
