"""Gemma 2 weight-loader unit tests — synthetic state dict."""
import pytest
import torch
...
def test_load_hf_gemma2_weights_into_layer():
    cfg = _config_tiny()
    blk = layer.build_gemma2_decoder_layer(cfg, layer_idx=0, max_seq=32)
    sd = _synthetic_sd(cfg, 0)
    layer.load_hf_gemma2_layer(blk, sd, layer_idx=0)
    # Spot-check three slots.
...
def test_load_hf_gemma2_missing_tensor_raises():
    cfg = _config_tiny()
    blk = layer.build_gemma2_decoder_layer(cfg, layer_idx=0, max_seq=32)
    sd = _synthetic_sd(cfg, 0)
    del sd["model.layers.0.input_layernorm.weight"]
    with pytest.raises(KeyError, match="missing tensors"):
        layer.load_hf_gemma2_layer(blk, sd, layer_idx=0)


def test_load_then_forward_gemma2():
