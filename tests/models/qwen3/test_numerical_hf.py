...
range -- semantics don't matter, only that both implementations see the same
input.

Adapted to transformers 5.10.2:
  - Qwen3Config no longer exposes a flat ``rope_theta`` key in ``to_dict()``;
    it lives under ``rope_parameters``. ``Qwen3Config.from_hf_dict`` handles
    both shapes.
  - ``Qwen3DecoderLayer.forward()`` now takes ``past_key_values`` (plural) and
    returns a single tensor (not a tuple).
  - We pin ``attn_implementation="eager"`` so HF runs the reference eager
    attention path (matches our SDPA-equivalent reference math).
"""
import os
import pytest
import torch
...
def test_layer0_forward_matches_hf(hf_model):
    ...
    with torch.no_grad():
        ...
        # 4D additive causal mask with -inf above the diagonal -- matches
        # eager_attention_forward in transformers 5.10.2 which adds this
        # directly to attn_weights.
