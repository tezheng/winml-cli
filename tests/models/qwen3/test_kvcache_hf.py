...
as HF for Qwen3-0.6B.

Runs on CPU, fp32 compute. Drives HF's DynamicCache through one prefill step and
one decode step, then drives our ContiguousKVCache through the same two-step
sequence, and asserts both layer outputs match at atol=5e-4.

Adapted to transformers 5.10.2:
  - ``Qwen3DecoderLayer.forward()`` takes ``past_key_values`` (plural) and
    returns a single tensor (not a tuple) -- no ``[0]`` indexing.
  - ``output_attentions`` kwarg is gone.
  - ``attn_implementation="eager"`` pinned so HF runs the reference eager
    attention path that matches our SDPA-equivalent math (see T17).
"""
import os
import pytest
import torch
...
def test_prefill_then_decode_layer0(hf_model):
    ...
    with torch.no_grad():
        ...
        # 4D additive causal mask with -inf above the diagonal (matches T17
        # eager attention reference).
        ...
        # No attention mask for single-token decode -- the cache provides the
        # full context and there's nothing to mask within a length-1 query.
        hf_dec_out = hf_layer(
            decode_h,
            attention_mask=None,
    ...
    with torch.no_grad():
        # Re-derive token embeddings for the API path (same weights, but make the
        # data dependency explicit rather than relying on HF temporaries).
    ...
    print(f"prefill max_abs_diff={pre_diff:.3e}, decode max_abs_diff={dec_diff:.3e}")
